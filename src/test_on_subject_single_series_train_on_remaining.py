...
for config_index in range(0, configuration.get_config_size()):
    ...
    for test_subject in list(range(1, 13)):
        for test_series in train_series:
            ...
            for train_subject in train_subjects:
                ...
                if train_subject == test_subject:
                    tmp_lst = list(range(1, 9))
                    del tmp_lst[test_series - 1]
                    series = str(tmp_lst)
                train_files = glob(dataset_path + 'subj%d_series%s_data.csv' % (train_subject, series))
                ...
                feature_extractor, ex_file = train_feature_extractor(train_raw, picks, configuration,
            ...
            test_file = dataset_path + 'subj%d_series%d_data.csv' % (test_subject, test_series)
            # read test data
            test_labels = test_file.replace('_data', '_events')
            test_raw = creat_mne_raw_object(test_file, read_events=False)
            ...
            del test_raw
            # read ids
            ids = np.array(pd.read_csv(test_file)['id'])
            ids_tot.append(ids)
            ...
            actual_events_proba = np.transpose(np.asarray(extract_events_from_file(test_labels)[0], dtype=np.float32))
